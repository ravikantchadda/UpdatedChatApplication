...
@interface ChatMessageVC ()<QMChatServiceDelegate,QMChatConnectionDelegate,UITextViewDelegate,QMChatAttachmentServiceDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIActionSheetDelegate,QMChatCellDelegate>
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(func_toPlayVideo)
                                                 name:@"playVideo"
                                               object:nil];
    ...
    self.tempMessages =[[NSMutableArray alloc] init];
    [self.tempMessages addObjectsFromArray:self.items];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (self.shouldUpdateNavigationStack) {
        ...
        [self.navigationController.viewControllers enumerateObjectsUsingBlock:^(UIViewController* obj, NSUInteger idx, BOOL *stop) {
            if ([obj isKindOfClass:[LoginVC class]] || [obj isKindOfClass:[ChatVC class]]) {
...
- (NSAttributedString *)topLabelAttributedStringForItem:(QBChatMessage *)messageItem {
    ...
    if (self.dialog.type != QBChatDialogTypePrivate) {
        ...
        topLabelText = (user != nil) ? user.fullName : [NSString stringWithFormat:@"%lu",(unsigned long)messageItem.senderID];
    }
...
- (void)collectionView:(QMChatCollectionView *)collectionView configureCell:(UICollectionViewCell *)cell forIndexPath:(NSIndexPath *)indexPath
{
    ...
    [(QMChatCell *)cell containerView].tag = indexPath.row;
    ...
    if ([cell conformsToProtocol:@protocol(QMChatAttachmentCell)]) {
        ...
        if (message.attachments != nil) {
            ...
            if ([attachment.type isEqualToString:@"video"]) {
                [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%d",(int)indexPath.row] forKey:@"tag"];
            }else{
                [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%d",(int)indexPath.row] forKey:@"tag"];
            }
            ...
            [[ServicesManager instance].chatService.chatAttachmentService getImageForChatAttachment:attachment completion:^(NSError *error, UIImage *image) {
                ...
                if (error != nil) {
                    ...
                    if (image != nil) {
                        ...
                        [(QMChatCell *)cell containerView].tag = indexPath.row;
                        if ([attachment.type isEqualToString:@"video"]) {
                             [(UICollectionViewCell<QMChatAttachmentCell> *)cell setPlayImage:1];
                            
                        }
                        else{
                        
                            [(UICollectionViewCell<QMChatAttachmentCell> *)cell setPlayImage:0];
...
-(void)func_toPlayVideo{

 NSString *strTag =    [[NSUserDefaults standardUserDefaults]objectForKey:@"playtag"];
    
    NSLog(@"________%d",strTag.intValue);
    
     NSLog(@"________%@",self.tempMessages);
  
    NSLog(@"New________%d",(int)self.tempMessages.count);

    
    if (self.tempMessages.count==0) {
        return;
    }
    
    
    QBChatMessage* message = self.items[strTag.intValue];
    if (message.attachments != nil) {
        QBChatAttachment* attachment = message.attachments.firstObject;
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        CustomMoviePlayerViewController *moviePlayer = [[CustomMoviePlayerViewController alloc] initWithPath:path];
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        ImageViewerVC *_imgViewerVC = [[ImageViewerVC alloc] initWithNibName:@"ImageViewerVC" bundle:[NSBundle mainBundle]];
                        
                        [[NSUserDefaults standardUserDefaults]setObject:path forKey:@"imagepath"];
                        [self presentViewController:_imgViewerVC animated:YES completion:nil];
...
- (void)collectionView:(QMChatCollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
...
- (void)chatService:(QMChatService *)chatService didAddMessageToMemoryStorage:(QBChatMessage *)message forDialogID:(NSString *)dialogID {
    if ([self.dialog.ID isEqualToString:dialogID]) {
        ...
        [self.tempMessages removeAllObjects];
        [self.tempMessages addObjectsFromArray:self.items];
        [self refreshCollectionView];
...
-(void)didPickAttachmentVideo:(NSData *)videoData{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        // NSData *binaryImageData = UIImagePNGRepresentation([UIImage imageNamed:@"chatPrivateIcon.png"]);
        NSString *imageName = [NSString stringWithFormat:@"video.mp4"];
        NSString *pathOfVideo = [basePath stringByAppendingPathComponent:imageName];
       ...
       // UIImage* newImage = image;
        ...
        [[ServicesManager instance].chatService.chatAttachmentService sendMessage:message
                                                                toDialog:strongSelf.dialog
                                                                  withChatService:[ServicesManager instance].chatService withAttachedVideoData:videoData
