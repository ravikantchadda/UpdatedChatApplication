...
- (void)createChatWithName:(NSString *)name completion:(void(^)(QBChatDialog* dialog))completion
{
    ...
    if (selectedUsers.count > 1) {
        ...
        [dialog addButtonWithTitle:@"Create" andHandler:^(NSInteger buttonIndex, UIAlertDialog *dialog) {
            ...
            [ServicesManager.instance.chatService createGroupChatDialogWithName:[dialog textFieldText] photo:nil occupants:selectedUsers completion:^(QBResponse *response, QBChatDialog *createdDialog) {
                if (response.success) {
        ...
        UIAlertController *_alertController = [UIAlertController alertControllerWithTitle:@"Alert!" message:@"Select atleast two users" preferredStyle:UIAlertControllerStyleAlert];
